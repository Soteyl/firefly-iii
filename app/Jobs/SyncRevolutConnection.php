...
class SyncRevolutConnection implements ShouldQueue
{
    use Dispatchable;
    ...
    public function handle(RevolutSyncService $revolutSyncService): void
    {
        ...
        try {
            $revolutSyncService->syncConnection($connection, (string) $run->trigger, $run);
        } catch (Throwable $e) {
            Log::warning(sprintf('Queued Revolut sync job failed for bank connection #%d: %s', $connection->id, $e->getMessage()));
